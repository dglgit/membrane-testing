...
modeller.addSolvent(forcefield, numAdded=726)
...
#            modeller.delete([residue])
#            break
# Add solvent (water) and ethanol molecules
#modeller.addSolvent(forcefield, model='tip3p', numAdded=30)
#count=0
#ethanol_positions_list=[]
#start=-2.1
#stop=2.1
#step=0.4
#for side in [-7,-5.5,-4,4,5.5,7]:
#    for i in np.arange(start,stop,step):
#        for j in np.arange(start,stop,step):
#            modeller.add(ethanol_topology,ethanol_positions+Vec3(i,j,side)*nanometers)
#            print(count)
            #ethanol_positions_list.append(ethanol_positions+Vec3(i,j,side)*nanometer)
#            count+=1
#            pass
#print('done appending')
#ethanol_topologies=[ethanol_topology]*count
#off_topologies=FFTopology.from_openmm(modeller.topology, unique_molecules=[ethanol])
#off_topologies.add_molecules([ethanol]*count)

#modeller.topology=off_topologes.to_openmm()
#modeller.positions.extend(ethanol_positions_list)
#modeller.add(ethanol_topologies,ethanol_positions_list)
#print(f'added {count} ethanols')
